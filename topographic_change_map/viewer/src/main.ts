...
const query=new URLSearchParams(location.search);const requestedGrid=query.get("grid")==="10m"?"10m":"32m";productGrid.value=requestedGrid;
productGrid.addEventListener("change",()=>{const next=new URL(location.href);if(productGrid.value==="10m")next.searchParams.set("grid","10m");else next.searchParams.delete("grid");location.assign(next);});
...
function build(data:GridData):void {
  grid=data; const {width,height,resolutionM}=grid;const span=Math.max(width,height)*resolutionM;camera.position.set(span*.38,-span*.58,span*.45);camera.far=span*10;camera.updateProjectionMatrix();scene.fog=new THREE.FogExp2(0x071018,1.35/span);positions=new Float32Array(width*height*3);colors=new Float32Array(width*height*3);const indices:number[]=[];
...
fetch(requestedGrid==="10m"?"./data/surface-grid-10m.json":"./data/surface-grid.json").then(response=>{if(!response.ok)throw new Error(`Data request failed: ${response.status}`);return response.json();}).then(build).catch(error=>{viewport.innerHTML=`<div class="error"><h2>Terrain data unavailable</h2>${String(error)}</div>`;});
