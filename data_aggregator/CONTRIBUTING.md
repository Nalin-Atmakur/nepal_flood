...
3. Pipeline:
   ...
   .venv/bin/python process_data.py            # RAW + ARCHIVE → DERIVED
...
4. Website:
   ...
   ```
   `npm run build` must pass with and without `.env.local`; with it absent every block renders its empty state.
...
3. Save one real response as `pipeline/tests/fixtures/<id>.<ext>`, anonymised (replace names/phones with placeholders; keep the shape). Add `test_<id>` to `pipeline/tests/test_normalisers.py` asserting row counts and a couple of values.
...
7. Add a paragraph to `pipeline/docs/pull_external_data/05-sources.md` (endpoint, shape, quirks). Full walkthrough with the same numbering: `pipeline/docs/pull_external_data/06-adding-a-source.md`.
8. Run `pull_external_data.py` once by hand; confirm a `pulls` row with `ok = true` and rows in the target table; check `/sources` shows it.

## 3. Add a processing step

Steps are numbered in the code, the docs and the DERIVED tables they write (`db/docs/04-derived.md`).

1. Create `pipeline/processing/<nn>_<name>.py` (or `<name>.py` registered at position `nn` in `process_data.py`) exposing `run(db, state) -> dict` that is idempotent: re-running with no new input must be a no-op.
...
5. Write `pipeline/docs/process_data/<nn>-<name>.md`: stage diagram, inputs → tables → outputs, failure behaviour. Numbers match the code (`03-ledger.md` ↔ `processing/ledger.py` ↔ step ③).
...
2. Create `web/messages/<code>.json` with **every** key from `en.json`; the parity test in `web/tests` fails on a missing key. Numbers stay Latin in all languages; Devanagari-script languages need `line-height ≥ 1.6` (already in the design tokens).
3. `web/middleware.ts`: the Accept-Language redirect picks the new code up from `LANGS`; check the fallback order.
...
1. New file `db/migrations/006_<topic>.sql`. **Never edit `001`–`005`** — `db/apply.py` refuses a changed applied file and would otherwise leave the ledger lying (`db/docs/07-applying-migrations.md`).
...
3. Never commit: `.env`, `.env.*`, `pipeline/snapshots/`, `pipeline/_state.json`, `node_modules/`, `.next/`, `.vercel/`, build output, database dumps. The repo-wide `.gitignore` also blocks `*.csv`, `*.xlsx`, `*.parquet`, `data/**/*.json` and imagery as a personal-data firewall; the gazetteer's `places.csv` is the deliberate exception — force-add it only after confirming it contains no personal data.
...
5. Push to `origin/main`; deploy only if `web/` changed (`docs/runbook.md` §7).
...
- test fixtures (anonymise before saving; keep the shape, replace the values);
- log lines (`run.log`, Vercel logs, browser console) — log ids and counts, not text;
...
The site is volunteer-run and not an official source. Every page carries the official channels (Police 1155 · Tourist Police 1144 · MoFA ECR · Red Cross 1130 · NEOC 1149); nothing we build replaces reporting to them.
...
```
   ...
   pipeline/README.md + pipeline/docs/
       pull_external_data/01–07    one numbered file per stage of the pull script
       process_data/00–08          one numbered file per processing step, numbers match the code
   web/README.md + web/docs/01–12  app architecture, design system, i18n, each route, deploy
