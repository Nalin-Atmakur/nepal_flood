...
Every table and view in the Supabase project, generated by reading `db/migrations/001…005`. The migrations are the truth; when they change, change this file in the same commit (see `CONTRIBUTING.md`, "Change the schema").
...
```
   ...
   ╚═══════════════════════╤════════════════════════╝   ╚═══════════════════╤═══════════════════╝
                           ...
                           └──────────────── process_data (service key) ◄───┘
                                        ① resolve ② dedup ③ ledger ④ latest ⑤ stats ⑥ findings
                                                          │
                                                          ▼
                       ╔═════════════════════════ DERIVED ═════════════════════════╗
...
| the two pipeline scripts | `service_role` | `pipeline/.env` on the machine running cron | everything (bypasses RLS) |
...
| Column | Type | Meaning |
|---|---|---|
| `filename` | text PK | e.g. `003_derived.sql`, `sources.sql` |
...
| `scope` | text, default `'national'` | `national` · `district:<name>` · `nationality:<iso>` · `place:<place_id>` · `project:<name>` |
...
Unique `(publisher, metric, scope, as_of, value)` — the normaliser upsert key. Index `(publisher, metric, scope, as_of desc)`. Writes: normalisers in `pull_external_data`. Reads: `process_data` ④ → `figures_latest`. RLS: service only.
...
Index `(published_at desc)`. Writes: `pull_external_data` (rows), `process_data` ① (`places`, `extracted`). Reads: `process_data`; public only through `v_articles_recent`. RLS: table service only.
...
| `last_contact_at` | timestamptz | latest contact out of the place |
| `telecom_restored` | boolean | |
| `phones` | text | display: `yes (since 28 Aug)` · `no` · `partial` |
...
**`place_timeline`** — "Status, day by day" on the place page (`process_data` ③).
...
| Column | Type | Meaning |
|---|---|---|
| `id` | text PK | `wave_time_to_port` · `wave_speed` · `galchhi_rise` · `bodies_downstream_km` · `missing_counts_divergence` · `reports_total` · `reports_last_hour` · `places_with_unknown` · `gauges_alive` · `next_flying_window` … |
...
RLS: `figures_latest_public`, `place_status_public`, `place_timeline_public`, `stats_public`, `report_counts_public` — select for `anon`, `authenticated`. Writes: `process_data` only.
...
`db/apply.py` runs every `db/migrations/*.sql` then every `db/seed/*.sql`, in filename order, through the Management API query endpoint, and records each file:
...
`reports_archive` has a `for update` policy for the owner (`reports_own_withdraw`), but the only update a user may make is a withdrawal. The trigger `reports_archive_guard` (`004_rls.sql`) enforces that:
...
```
   ...
   reports_archive_guard_update()  before update, for each row
        │
        ├── jwt role = service_role, or current_user = postgres  → allow the update as written
        │
