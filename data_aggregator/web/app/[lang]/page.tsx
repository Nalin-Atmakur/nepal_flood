...
import { getArticles, getFlyingWindows, getGauges, getLiveCounts, getNationalFigures, getPlaces, getPlaceStatuses, getStats } from "@/lib/queries";
...
/**
 * Home — composes the blocks in design order: scoreboard → 01 corridor → 02 numbers → 03 side by side →
 * 04 places → 05 add → 06 river & weather → 07 latest → share. ISR every 5 minutes. See web/docs/05-home-blocks.md.
 */
...
export default async function Home({ params }: { params: Promise<{ lang: string }> }) {
  const lang = asLang((await params).lang);
  const [live, statuses, refs, stats, figures, gauges, windows, articles] = await Promise.all([
    getLiveCounts(),
    getPlaceStatuses(),
    getPlaces(),
    getStats(),
  ...
  return (
    <main>
      ...
      <section data-block="places" data-n="04" className="max-w-[1280px] mx-auto px-4 md:px-7 mt-7" aria-labelledby="sec-places">
        <SectionHead n="04" title={<span id="sec-places">{t(lang, "sec.places")}</span>} sub={<span className="hidden md:inline">{t(lang, "sec.places_sub")}</span>} align="center" />
