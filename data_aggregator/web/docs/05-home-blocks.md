...
| — | FloodVideos (inside 01, under the controls) | `blocks/FloodVideos.tsx` → `VideoTile` (client, click-to-play) | `lib/videos.ts` `FEATURED_VIDEOS` (three, curated in the repo) + `places` for the place chips (docs/18) | renders nothing when the list is empty |
