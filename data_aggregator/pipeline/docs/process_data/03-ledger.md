...
## Timeline (`place_timeline`, key `place_id, day, kind`)

Since migration 010 (30 Aug 05:30) a line is keyed by its **kind** — the template key below, or `article:<md5(title)>`
for headlines — not by its text. A re-run therefore *replaces* the day's line ("River gauge live (2.59 m)" overwrites
"(2.55 m)") instead of adding one; `collapse_timeline()` applies the same rule in memory before the upsert
(`tests/test_ledger_timeline.py`). The migration dropped 311 legacy templated rows, regenerated by the next step ③ run
(877 rows, 0 duplicate keys on 30 Aug 04:27 UTC).
