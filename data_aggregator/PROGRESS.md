...
3. **Report form**: archive-only before distribution. Original text/contact/place/files are stored privately; the automated pipeline never reads, models, summarises, matches, counts or publishes them. The receipt and My Info show only Stored/Received/Withdrawn. Public submission-activity counters remain content-free.
...
## Cycle log
- 15:05–15:25 BST — Archive-only family boundary implemented end to end (D-074): fail-closed runtime flag; every family read/derived path gated; OpenAI remains public-source-only; `reports_total`/`report_counts` publication removed; immediate private-storage receipt and Received/Withdrawn My Info in EN/NE/HI; no schema migration. Live pre-distribution audit found and transactionally removed only synthetic derived residue (2 count buckets, 1 timeline, 1 form entity/event, 3 obsolete stats, 15 affected status snapshots), retaining all 12 withdrawn raw test rows. Public-source rebuild: family read/write 0, no `anonymise` call; health now `reports_anon=0`, `report_counts=0`, OK. Historical assessment/audit files remain pinned records.
- 11:07 BST — /sources renders cards on phones (11f51a6; e2e 23) — deployed. One pipeline tick run by me (09:50–10:05 UTC) so live numbers are fresh; the owner starts the loop themselves.
...
- 12:35 BST 30 Aug · Corridor pass on the owner's 11:48–11:54 screenshots (D-058): discs = object pads (incl. real bridges) → removed; place pills 1.1 units + **Names** toggle; feed 3–6 rows; physics: taken → carried whole (≤ 6 s, tumbling) → broken → carried on → off the east edge (waterfall spray). 115 unit · 23 e2e green; deployed 12:45 BST, live smoke ok.
- 13:05 BST 30 Aug · Water as a level fill of the valley (moulds to the walls) + translucent by X-ray amount (D-059); zoom toward the cursor / pinch midpoint (D-060). 119 unit · 23 e2e green; deploying.
- 13:35 BST 30 Aug · Real footage block under the corridor (D-061): nine verified clips as click-to-play posters, en/ne/hi captions, place chips, fact-check note, "Have footage?" → form; `web/docs/18-flood-videos.md` (add-a-clip steps + fake checklist). Building/deploying.
