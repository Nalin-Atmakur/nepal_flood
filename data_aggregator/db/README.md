...
```
   ...
   └───────────┬────────────┘          └───────────┬────────────┘
               ...
               └────────────►──────────────────────┤
                      ...
                      │          report_counts event_timeline│
                      │          digest figure_series        │
...
| DERIVED (computed) | public `figures_latest`, `place_status`, `place_timeline`, `stats`, `report_counts`, `event_timeline`, `digest`, `figure_series` · private `entities`, `entity_events`, `dedup_queue`, `findings` | `process_data` (+ the `event_timeline` seed) | website (public), `process_data` (private) |
...
| `docs/04-derived.md` | `003_derived.sql` + `006_story_and_digest.sql` + `007_series.sql` — which step writes which table, the views, the site contract |
| `docs/05-rls.md` | `004_rls.sql` (+ `006_pipeline_additions.sql`, the trigger's service-role detection) — policy matrix, the trigger, views-as-owner caution, tests |
...
├── migrations/          001_archive · 002_raw · 003_derived · 004_rls · 005_realtime_storage · 006_pipeline_additions · 006_story_and_digest · 007_series  (applied in name order; never edited after applying)
├── seed/                gen_sources.py → sources.sql (from ../sources.yaml) · places.sql (from ../gazetteer/places.csv via ../gazetteer/to_sql.py) · event_timeline.sql (17 hand-written first-hours rows)
...
4. `../pipeline/.venv/bin/python apply.py` — runs `migrations/*.sql` then `seed/*.sql` in filename order, skipping files already in `_migrations` with the same checksum (`make db` from `data_aggregator/` is the same). Live ledger on 30 Aug 2026: eight migrations + three seeds.
...
2. `../pipeline/.venv/bin/python seed/gen_sources.py` and/or `../pipeline/.venv/bin/python ../gazetteer/to_sql.py` (`make db-seed` does both and applies).
...
2. `../pipeline/.venv/bin/python -m pytest tests -q` from `db/` (`make db-test`). `conftest.py` forces IPv4 for the Supabase host — the `*.supabase.co` AAAA record does not connect on some networks.
...
1. New file `migrations/008_<topic>.sql` (next free number); idempotent statements (`if not exists`, `create or replace`, `drop policy if exists` before `create policy`).
...
5. Commit the migration, docs and test together. Never edit an applied migration again (`001`–`007`).

## Rules

- Never add a policy that lets `anon`/`authenticated` select from `reports_archive` (other than own rows), `raw_pulls`, `figures`, `articles` (table), `reports_anon`, `pulls`, `entities`, `entity_events`, `dedup_queue`, `findings`, `_migrations`. (Postgres still shows default table grants for `anon` on public tables; RLS with no policy is what denies — the tests in `tests/test_rls.py` check the effect, not the grants.)
